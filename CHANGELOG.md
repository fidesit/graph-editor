...
- **`graphChange` not emitted on edge path type switch** — Changing the edge path type (straight/bezier/step) via the toolbar or `setEdgePathType()` now correctly emits a `graphChange` event, enabling consumers to react to visual state changes.

## [1.0.21] - 2026-03-06

### Fixed

- **Resize handle visible in readonly mode** — The node resize handle is now hidden when the editor is in readonly mode (via `readonly` input or `interaction.readonly` config).
